...
public class TicketSharedServiceImpl implements TicketSharedService {
    ...
    private void validateFlightDepartureDateForRoundTripFlight(Flight outwardFlight,
        ...
        LocalDateTime outwardArriveDateTime = DateTimeUtil.toLocalDateTime(
                outwardFlight.getDepartureDate(), outwardFlight.getFlightMaster().getArrivalTime());
        // 搭乗可能時刻
        LocalDateTime reserveAvailableDateTime =
                outwardArriveDateTime.plusMinutes(reserveIntervalTime);

        // 復路のフライト出発時刻
        LocalDateTime homewardDepartureDateTime =
                DateTimeUtil.toLocalDateTime(homewardFlight.getDepartureDate(),
    ...
    public boolean isAvailableFareType(FareType fareType, Date depDate) {
        ...
        return !(sysDate.isBefore(rsrvAvailableStartDate) || sysDate.isAfter(rsrvAvailableEndDate));
    }
